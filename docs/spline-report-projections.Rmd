...
Also note that we are now fitting to incident case and death data that is smoothed as a 7-day moving average.
...
3. Return to normal: both $\phi$ and $\psi$ return to one over a seven day period and then remain there.
...
I am also not super happy about the amount of uncertainty surrounding projections.
I think the CI bands should be much wider.
I will explore projection from a set of trajectories from initial conditions rather than choosing the best trajectory (base on match between model and observations at the final observation time) and simulating all replicates from the same initial conditions.
Unfortunately, this procedure only led to 1 parameter set within 2 log likelihood points of the MLE.
